	...
	Query: {
		...
		**/
		getAllTodos: async (_, __, { req }) => {
			...
			if(!_id) { return([])};
			const todolists = await Todolist.find({owner: _id}).sort({updatedAt:-1});
	...
	Mutation: {
		...
		**/
		...
		sortItems: async (_, args) => {
			...
			const found = await Todolist.findOne({_id:listId});
			let listItems = found.items;
			let newListItem = listItems.slice();
			if (type === 0){//sort by task name
				listItems.sort((a, b) => a.description.localeCompare(b.description));
				if (JSON.stringify(listItems) == JSON.stringify(newListItem)){
			...
			if (type === 1){
				...
				if (JSON.stringify(listItems) == JSON.stringify(newListItem)){
					listItems.sort((a, b) => b.due_date.localeCompare(a.due_date));
				}
			}
			if (type === 2){
				listItems.sort((a, b) => a.completed == b.completed ? 0 : (a.completed ? -1:1));
				if (JSON.stringify(listItems) == JSON.stringify(newListItem)){
					listItems.sort((a, b) => b.completed == a.completed ? 0 : (b.completed ? -1:1));
				}
			}
			if (type === 3){
